...
// levelOnly：等级类成就（按当前 exp 重算等级判定，admin 降 exp 后可回退）；行为类成就（first_msg/msg_100 等）一次性解锁不回退
export const ACHIEVEMENTS = [
  ...
  { id: "level_5",    icon: "⭐", name: "崭露头角",  desc: "等级达到 Lv.5",       levelOnly: true, check: (s, l) => l >= 5 },
  { id: "level_10",   icon: "🌟", name: "声名鹊起",  desc: "等级达到 Lv.10",      levelOnly: true, check: (s, l) => l >= 10 },
];

// 检查用户是否新解锁成就；有则写入 user.achievements 并持久化。返回新解锁的成就 id 数组。
// 🔒 安全修复（F5）：等级类成就（levelOnly）随当前 exp 重算等级——admin 降低 exp 后若不再满足条件则从成就列表移除（此前只增不减）。
// 行为类成就（first_msg/msg_100/checkin_1 等）保持一次性解锁，不回退。
export async function checkAchievements(reg, name, user) {
  try {
    ...
    for (let a of ACHIEVEMENTS) {
      if (a.levelOnly && unlocked.has(a.id) && !a.check(stats, level)) {
        unlocked.delete(a.id);
        changed = true;
        continue;
      }
