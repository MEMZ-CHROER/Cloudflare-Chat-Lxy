...
// 用户菜单响应式状态（menu.js showUserMenu 双轨通过 window.__navSetUserMenu 写这里）
...
const NavBar = {
  ...
  setup() {
    ...
    function lazy(action) {
      return () => import('./' + action.file + '.js').then((m) => {
    ...
    const moreActions = {
      ...
      scheduler: () => { document.querySelector('#schedule-btn') && document.querySelector('#schedule-btn').click(); },
    ...
    const bottomItems = [
      ...
      { key: 'more', icon: () => moreOpen.value ? '✕' : '···', title: '更多', fn: () => { moreOpen.value = !moreOpen.value; } },
    ];

    return {
      moreOpen, soundMuted, isDark, t,
      toggleSound, toggleDark, floatActions, moreActions, moreItems, bottomItems,
    };
  },
  template: `
...
export function initNav() {
  ...
  const hideIds = ['sound-toggle', 'dark-toggle', 'search-toggle', 'settings-toggle', 'music-toggle', 'tag-warehouse-btn', 'more-menu-btn', 'more-menu-backdrop', 'more-menu-panel', 'mobile-bottom-bar'];
  ...
  // menu.js 双轨写用户菜单状态
  window.__navSetUserMenu = (payload) => { Object.assign(userMenuState, payload); };
  Vue.createApp({
    name: 'NavRoot',
