...
export async function genWebhook(room) {
  ...
  try {
    ...
    let url = location.origin + "/api/webhook/" + encodeURIComponent(room) + "?secret=" + d.secret;
    let curl = 'curl -X POST "' + url + '" -H \'Content-Type: application/json\' -d \'{"content":"你好"}\'';
    ...
    detail.innerHTML = '<div style="margin:10px 0 14px 0;padding:12px;background:#f6f8fa;border:1px solid #e1e4e8;border-radius:6px;font-size:13px">'
      + '<div style="margin-bottom:6px"><strong>Webhook URL（含密钥，请保密）：</strong></div>'
      + '<div style="word-break:break-all;font-family:monospace;background:#fff;padding:8px;border-radius:4px;border:1px solid #e1e4e8">' + url + '</div>'
      + '<div style="margin:10px 0 6px 0"><strong>调用示例：</strong></div>'
      + '<div style="word-break:break-all;font-family:monospace;background:#fff;padding:8px;border-radius:4px;border:1px solid #e1e4e8;white-space:pre-wrap">' + curl + '</div>'
      + '<div style="margin-top:8px;color:#888">可选字段：sender（发送者名，默认 Webhook）、channel（目标频道，默认 general）；secret 也可放 X-Webhook-Secret 请求头</div>'
